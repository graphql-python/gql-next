...
from gql.clients import Client, AsyncClient
...
@classmethod
def execute(cls, {vars}, on_before_callback: Callable[[Mapping[str, str], Mapping[str, str]], None] = None):
    client = Client({url})
    response_text = client.call(cls.__QUERY__, on_before_callback=on_before_callback)
    return cls.from_json(response_text)

@classmethod
async def execute(cls, {vars}, on_before_callback: Callable[[Mapping[str, str], Mapping[str, str]], None] = None):
    client = AsyncClient({url})
    response_text = await client.call(cls.__QUERY__, on_before_callback=on_before_callback)
    return cls.from_json(response_text)
"""
...
class DataclassesRenderer:
    ...
    def __render_operation(self, node: MappingNode, indent: int = 0):
        ...
        # Execution functions
        vars = ', '.join([f'{vdef.variable.name.value}: {self.__variable_type_to_python(vdef.type)}' for vdef in graphql_node.variable_definitions])
        for line in OPERATION_EXEC_FUNC_TEMPLATE.format(vars=vars, url=self.config.schema).split(os.linesep):
            yield ind1 + line
